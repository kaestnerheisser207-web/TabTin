 ...
 * `$MUSE_WORKSPACE/foo.md` etc. without the agent inlining long absolute
 * paths.
 *
 ...
 *
 * §17.6 D4.b：env 变量名从 `MUSE_SESSION_ID` 改名 `MUSE_THREAD_ID`，
...
export function buildTabtinRuntimeEnv(
  ...
  if (context.workspaceRoot) env.MUSE_WORKSPACE = context.workspaceRoot;
  if (context.threadId) env.MUSE_THREAD_ID = context.threadId;
  if (context.agentRunId) env.MUSE_AGENT_RUN_ID = context.agentRunId;
  if (context.toolUseId) env.MUSE_TOOL_USE_ID = context.toolUseId;
  if (agentId) env.MUSE_AGENT_ID = agentId;
  if (spaceId) env.MUSE_SPACE_ID = spaceId;
  if (organizationId) env.MUSE_ORGANIZATION_ID = organizationId;
  return env;
}
