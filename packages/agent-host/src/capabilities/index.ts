/**
 * `@muse/agent-host/capabilities` —— 平台目录类 Capability barrel。
 *
 ...
 * 故从 `@muse/agent-runtime` 的通用 capability/core 迁到共享宿主包 `@muse/agent-host`。
 *
 * agent-runtime 的 capability/core 只保留真正通用的能力（filesystem / shell 等）；
 * 本模块经 `@muse/agent-runtime` 单向跨包 import 其 `CapabilityBase` /
 * 引擎契约 / 召回 helper / skills 子系统。两宿主（electron / daemon）装配时从
 * `@muse/agent-host/capabilities` import 这三个 Cap。
 */
...
// ：show_widget 烤图（offscreen 渲染 + OSS 上传，直连 @muse/action-tools）
...
// ：本地文件产物的 muse:// 资源 URI 构造，从 agent-runtime
