...
//
// agent-runtime spawn CLI 子进程时注入 MUSE_THREAD_ID（业务对话 id）和
// MUSE_AGENT_RUN_ID（本轮 Agent run id）。THREAD_ID 的存在即代表「本次
...
//
// MUSE_AGENT_RUN_ID 是 rollback_agent_run / 回退预览精确追踪结构化资源
// 变更的锚点；MUSE_THREAD_ID 继续作为会话维度关联。
func setAgentContextHeaders(req *http.Request) {
	threadID := os.Getenv("MUSE_THREAD_ID")
	if threadID == "" {
		return
	}
	agentRunID := os.Getenv("MUSE_AGENT_RUN_ID")
	if agentRunID != "" {
		req.Header.Set("X-Tabtin-Agent-Run-Id", agentRunID)
	}
	toolUseID := os.Getenv("MUSE_TOOL_USE_ID")
	if toolUseID != "" {
