...
// 参数透传给随包分发的独立二进制 `muse-filegen`（PyInstaller 自包含，客户端免装
// Python），由后者用成熟库生成 xlsx/docx/pptx/pdf。
//
// 设计取向：生成实现全部收敛在 muse-filegen，Go 侧零生成逻辑——新增文件类型时
// Go 不改（--type 透传，由 muse-filegen 校验）。承接  的「外部工具生成 +
...
func fileGenBinaryName() string {
	if runtime.GOOS == "windows" {
		return "muse-filegen.exe"
	}
	return "muse-filegen"
}

// resolveFileGenBinary 定位 muse-filegen：先查 PATH（Electron CLI Server 已把其
...
func resolveFileGenBinary() (string, error) {
	...
	if err == nil {
		...
		candidates := []string{
			...
			// 打包：Resources/tabtin-cli-go/dist/muse → Resources/muse-filegen-python/dist/muse-filegen
			filepath.Join(dir, "..", "..", "muse-filegen-python", "dist", name),
			// dev：packages/tabtin-cli-go/dist/muse → packages/muse-filegen-python/dist/muse-filegen
			filepath.Join(dir, "..", "..", "muse-filegen-python", "dist", name),
		}
	...
	return "", fmt.Errorf("找不到文件生成器 %s，请确认随包分发的 muse-filegen 已构建并在 PATH 上", name)
}
...
由随包分发的 muse-filegen 二进制完成（客户端无需 Python）：create 从 JSON spec
...
实现：透传给随包分发的 muse-filegen 二进制生成（不联网、不写远端），Go 侧不含
任何生成逻辑——新增文件类型时本命令无需改动（--type 由 muse-filegen 校验）。
...
调用 present_to_user 才会在聊天里出现卡片。`,
		...
		DryRun: func(ctx *cmdutil.RunContext) *cmdutil.DryRunPlan {
			...
			return cmdutil.NewDryRunPlan().
				Desc(fmt.Sprintf("本地生成 %s 文件到 %s（调用随包分发的 muse-filegen，不联网、不写远端）", ft, out)).
				Step("LOCAL", out).
				Set("tool", "muse-filegen").
				Set("file_type", ft)
		},
	...
	// 发现类命令同样代理到 muse-filegen——Agent 只需面对 `muse file`，
	// 不直接接触底层 muse-filegen 二进制。两者只读（RiskRead）、原样透传输出。
	// Layer: L2
	cmdutil.MustRegisterCommand(cmd, f, cmdutil.CommandDef{
		Use:   "schema",
...
透传给随包分发的 muse-filegen，Go 侧不内置 schema 定义。`,
...
透传给随包分发的 muse-filegen，Go 侧不含解析逻辑。`,
...
用于发现当前可生成哪些文件类型。
透传给随包分发的 muse-filegen；新增类型时本命令无需改动。`,
...
// muse-filegen，直接流式输出其 stdout/stderr 并透传退出码。
func runFileGenPassthrough(ctx *cmdutil.RunContext, args ...string) error {
	...
	if err != nil {
		return output.PrintErrorAndExit(output.ErrorEnvelope(
			...
			err.Error(),
			"构建文件生成器：bash packages/muse-filegen-python/build.sh",
...
// classifyFileGenError 把 muse-filegen 的 stderr 错误 envelope 映射到 CLI 的
